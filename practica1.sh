...
if [ $# -ne 0 ]; then
  echo "Buscant registres..."
  echo ""
  busqueda=$(grep $1 titles.cvs)
  echo ${busqueda:-"La paraula \"$1\" no correspon a cap registre."}
else
  echo "Inicialitzant el processament de dades..."
  echo ""
  # PAS 1
  ...
  rm pas1 pas2 Shows.csv Movies.csv Movies_net.csv Shows_net.csv
exit 1
fi